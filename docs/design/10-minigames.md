...
| Ocean Match-3 | `ocean-match3` | ❌ Não | Especificado, não implementado |

O `meteor-game` é atualmente o único minigame registrado no `MinigameRegistry`. Uma chamada a `getMinigame('ocean-match3')` lançará `MinigameNotFoundError` — o minigame ainda não está disponível. O ID `ocean-match3` permanece reservado na documentação para implementação futura.

## 2. Duração

| Propriedade | Valor |
|---|---|
| **Duração padrão** | 20 segundos |
| **Tolerância mínima** | 15 segundos |
| **Tolerância máxima** | 30 segundos |
| **Referência implementada** | MeteoroGame — 20s |

A duração é contada **após** a instrução curta e a contagem regressiva 3, 2, 1. O cronômetro só inicia quando o jogador assume o controle. Sessões longas são proibidas — se um minigame ultrapassar 30 segundos, o design deve ser repensado.
...
> ⚠️ O Ocean Match-3 **não está registrado** no MinigameRegistry. A especificação acima está documentada para referência futura. O registro será ativado na sprint de implementação.
...
|----|---------|------------|
| DMG-01 | Duração padrão de 20s para minigames | Consistência com MeteoroGame; tempo curto o suficiente para não cansar, longo o suficiente para desafio leve |
